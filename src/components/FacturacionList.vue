<template>
  <div class="container">
    ...
    <form @submit.prevent="crearFactura">
      ...
      <div v-for="(detalle, index) in factura.detalles" :key="index" class="mb-3">
        ...
        <div class="mb-2">
          ...
          <select v-model="factura.detalles[index].productoId" @change="actualizarPrecio(index)" class="form-select"
            required>
        ...
        <div class="mb-2">
          ...
          <input v-model="factura.detalles[index].cantidad" type="number" class="form-control" placeholder="Cantidad"
            min="1" required />
        </div>
        <div class="mb-2">
          <label for="precio" class="form-label">Precio:</label>
          <input v-model="factura.detalles[index].precio" type="number" class="form-control" placeholder="Precio"
            readonly />
        </div>
    ...
    <div v-if="facturaDetalles" class="mt-4">
      ...
      <button @click="imprimirFactura" class="btn btn-info">Imprimir Factura</button>
      <button @click="guardarComoPDF" class="btn btn-secondary">Guardar como PDF</button>
    </div>
    <!-- Componente para impresión -->
    <FacturaPrint v-if="facturaDetalles" :factura="facturaDetalles.factura" />
  </div>
</template>

<script>
import axios from 'axios';
import FacturaPrint from './FacturaPrint.vue';
...
export default {
  ...
  methods: {
    ...
    async guardarComoPDF() {
      const element = document.querySelector('.print-container');
      const canvas = await html2canvas(element);
      const imgData = canvas.toDataURL('image/png');
      const pdf = new jsPDF('p', 'mm', 'a4');
      pdf.addImage(imgData, 'PNG', 10, 10);
      pdf.save(`factura-${this.facturaDetalles.factura._id}.pdf`);
    }
  },
  created() {
    this.fetchClientes();
